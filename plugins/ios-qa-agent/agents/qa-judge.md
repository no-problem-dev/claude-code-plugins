...
2. **Final State の Screenshot** — 視覚的な妥当性、レイアウトを確認
3. **Actions Performed** — 操作が意図通り実行されたか
4. **Observations** — エラー、警告、異常の有無
...
| レベル | 評価基準 |
|--------|---------|
| high | 全ての期待項目で高品質な証拠がある。Final State の Key UI Elements が充実。複数の情報源から一貫性がある |
| medium | ほとんどの期待項目で証拠がある。ただし一部の要素について視覚的確認のみ、または単一の情報源に依存 |
| low | 期待項目の一部に対して証拠が不足している。または Status = completed だが Key UI Elements が貧弱。信頼度に疑問あり |
